use std::{array, collections::HashMap, ops::Range, sync::Arc};
...
use ranim_midi_visualizer_lib::config::{ColorBy, MetricBase};
use ranim_midi_visualizer_math::cyc_index::IndexCyc as _;
use waveform_utils::music::{Metric, Music, Note, NoteContainer as _};

use crate::{state::VideoConfigState, utils::RanimColor};

#[derive(IntoElement)]
pub struct PreviewArea {
    pub music: Arc<Music>,
    pub video_config: VideoConfigState,
    pub time: Metric,
}
...
impl PreviewArea {
    fn paint_preview(self, view_bounds: Bounds<Pixels>, window: &mut Window, cx: &mut App) {
        ...
        cx.read_entity(&visualizer_config, |visualizer_config, _cx| {
            ...
            let status_bar_origin = view_bounds.bottom_left() - point(px(0.), status_bar_height);
            ...
            let highlighted_keys: HashMap<_, _> = {
                ...
                match color_by {
                    Voice => notes_on
                        .map(|(pitch, [_, voice_idx])| (pitch, *note_colors.index_cyc(voice_idx)))
                        .collect(),
                    Staff => notes_on
                        .map(|(pitch, [staff_idx, _])| (pitch, *note_colors.index_cyc(staff_idx)))
                        .collect(),
                    KeyColor => notes_on
                        .map(|(pitch, _)| {
            ...
            window.paint_layer(key_bounds, |window| {
                ...
                let paint_black_key = |window: &mut Window, octave: i8, black_idx: u8| {
                    ...
                    let [fill_path, stroke_path] =
                        [PathBuilder::fill(), PathBuilder::stroke(stroke_width)].map(|mut pb| {
                            key_path(&mut pb, origin, black_key_size, corner_size, [None; 2]);
                            pb.build().expect("Failed to build path")
                        });
                    let key_color = highlighted_keys
                        .get(&pitch)
            ...
            macro paint_notes($window: expr, $note_container: expr, $cur_metric: expr$(,)?) {
                ...
                let note_y_bounds = |time_range: Range<Metric>| {
                    let note_height =
                        to_large_metric(time_range.end - time_range.start) as f32 * scroll_speed;
                ...
                let note_bounds = |time_range: Range<Metric>, note: Note| {
                    ...
                    bounds(point(origin_x, origin_y), size(width, height)).intersect(&scroll_bounds)
                };
                ...
                for ([staff_idx, voice_idx], time_range, &note) in visible_notes {
                    ...
                    let note_color = RanimColor(
                        match visualizer_config.note_config.color_by {
                            ...
                            KeyColor => *note_colors.index_cyc(is_black_key(note.pitch) as usize),
                        }
